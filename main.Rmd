...
plot_event <- function(event_df, x_lab, var_names = c("GDP", "CPI")) {
  event_df %>% 
    filter(index >=0 ) %>% 
    ggplot(aes(x = index)) +
    geom_line(aes(y = GDP*100, col="GDP")) +
...
```{r event-credit, fig.cap="Dynamics around credit cycle peaks and troughs"}
(data_leads_lags %>% 
  summarize_around_event(., peak_Credit == 1L, c("GDP", "CPI")) %>% 
  plot_event(., "Quarters after credit cycle peak"))  +
(data_leads_lags %>% 
  summarize_around_event(., trough_Credit == 1L, c("GDP", "CPI")) %>% 
  plot_event(., "Quarters after credit cycle trough"))   
```

```{r event-house, fig.cap="Dynamics around real estate cycle peaks and troughs"}
(data_leads_lags %>% 
  summarize_around_event(., peak_House == 1L, c("GDP", "CPI")) %>% 
  plot_event(., "Quarters after real estate cycle peak"))  +
(data_leads_lags %>% 
  summarize_around_event(., trough_House == 1L, c("GDP", "CPI")) %>% 
  plot_event(., "Quarters after real estate cycle trough"))   
```

```{r event-leverage, fig.cap="Dynamics around leverage cycle peaks and troughs"}
(data_leads_lags %>% 
  summarize_around_event(., peak_Leverage == 1L, c("GDP", "CPI")) %>% 
  plot_event(., "Quarters after leverage cycle peak"))  +
(data_leads_lags %>% 
  summarize_around_event(., trough_Leverage == 1L, c("GDP", "CPI")) %>% 
  plot_event(., "Quarters after leverage cycle trough"))   
```
...
```{r}
# helpers to create formula string with lags
...
# this returns a vector of strings when lhs is a vector
...
lmfit_credit_peak <- 
  ...
  batch_run_regress(., data = data_leads_lags %>% mutate_at(vars(contains("GDP")), ~.*100))
